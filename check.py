...
ONCHAIN_PUSH = "hourly"   # "hourly"=1시간 요약 1통만 / "critical"=중요만 즉시 / "all"=전부 즉시 / "off"=무음
MEGA_THRESHOLD = 1_000_000
EXPLORERS = {"1": "https://etherscan.io", "56": "https://bscscan.com",
             "8453": "https://basescan.org", "42161": "https://arbiscan.io",
             "137": "https://polygonscan.com", "10": "https://optimistic.etherscan.io",
             "43114": "https://snowtrace.io", "999": "https://hyperevmscan.io",
             "146": "https://sonicscan.org"}   # 이 이상 단일 이동은 지갑 무관 무조건 알림
...
def analyze_chain(txs, chain_state, pair_addr=""):
    ...
    for tx in txs:
        ...
        if amt >= WATCH_MIN:
            ...
            if f in WATCHLIST:
                lbl = WATCHLIST[f]
                _ex = chain_state.get("explorer", "")
                _link = f"\n🔗 {_ex}/tx/{tx['hash']}" if _ex else ""
                if pair_addr and t == pair_addr:
                    alerts.append(f"🚨👁 관찰지갑 [{lbl}] DEX 매도: {amt:,.0f} — 최우선 확인{_link}")
                elif looks_exchange(t):
                    alerts.append(f"🚨👁 관찰지갑 [{lbl}] 거래소 입금: {amt:,.0f} (매도 준비 가능){_link}")
                else:
        ...
        if pair_addr and t == pair_addr:
            ...
            if f in team:
                ...
                if amt >= TEAM_SELL_MIN:
                    _ex = chain_state.get("explorer", "")
                    _lk = f"\n🔗 {_ex}/tx/{tx['hash']}" if _ex else ""
                    alerts.append(f"🚨 팀 클러스터가 DEX에 매도: {short(f)} → 페어 {amt:,.0f} — 러그 경보 최상급{_lk}")
...
def main():
    ...
    if ETHERSCAN_KEY and CONTRACTS:
        for _ci, (chainid, contract) in enumerate(CONTRACTS):
            ...
            cst["explorer"] = EXPLORERS.get(str(chainid), "")
            if not cst.get("pair"):
    ...
    try:
        ...
        if _h == 0 and state.get("last_brief") != _today:
            ...
            nf = []
            for _cc in all_chain.values():
                nf += _cc.get("netflow_hist", [])
            if nf:
    ...
    if ONCHAIN_PUSH == "hourly" and chain_alerts:
        buf = state.setdefault("chain_digest_buf", [])
        buf += chain_alerts
        state["chain_digest_buf"] = buf[-60:]
    ...
    try:
        ...
        if ONCHAIN_PUSH == "hourly" and now0 - state.get("last_wallet_summary", 0) >= 3600:
            buf = state.get("chain_digest_buf", [])
            if wsum_all or buf:
                lines = ["⛓ 온체인 시간 요약"]
                crit = [m for m in buf if "🚨" in m]
                if crit:
                    lines.append("— 중요 이벤트 —")
                    lines += crit[:6]
                others = [m for m in buf if "🚨" not in m]
                if others:
                    from collections import Counter
                    def _emoji(m):
                        t = m.split("] ", 1)[-1]
                        return t[:2].strip()
                    kinds = Counter(_emoji(m) for m in others)
                    lines.append("— 기타 —")
                    lines.append(", ".join(f"{k}×{v}" for k, v in kinds.most_common(6)))
                if wsum_all:
                    lines.append("— 관찰지갑 흐름 —")
                    ranked = sorted(wsum_all.items(),
                                    key=lambda x: -(x[1]["out"] + x[1]["in"]))
                    for lbl, w in ranked[:10]:
                        bits = []
                        if w["out_n"]:
            ...
            state["chain_digest_buf"] = []
            for _c, _cst in all_chain.items():
                _cst["watch_summary"] = {}
    except Exception as _e:
        print(f"[wsum] {_e}")

    if ONCHAIN_PUSH in ("off", "hourly"):
        push_list = []
