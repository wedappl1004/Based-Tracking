...
def build_signals(cs, funding, oi, hl, liq, prev, cgd=None):
    ...
    if cgd.get("liq_long_now") is not None:
        if cgd["liq_long_now"] > cgd["liq_long_avg"] * 5 and cgd["liq_long_now"] > 50_000:
            S.append(("liq_long", f"💀 롱 대량청산: 1h ${cgd['liq_long_now']:,.0f} (평균의 {cgd['liq_long_now']/cgd['liq_long_avg']:.0f}배) — 바닥 근접 신호일 수 있음"))
        if cgd["liq_short_now"] > cgd["liq_short_avg"] * 5 and cgd["liq_short_now"] > 50_000:
            S.append(("liq_short", f"⚡ 숏 대량청산: 1h ${cgd['liq_short_now']:,.0f} — 스퀴즈 진행 중"))
...
def main():
    ...
    now0 = int(time.time())
    if (verdict != state.get("last_verdict")
